...
function displayFilmDetails(film) {
    ...
    if (titleElement) titleElement.textContent = film.name || 'Título não disponível';
    if (descriptionElement) descriptionElement.textContent = film.description || 'Descrição não disponível';
    ...
    try {
        ...
        if (type === 'EMBED') {
            ...
            if (decodedUrl && decodedUrl.startsWith('http')) {
                ...
                if (!newTab) {
                    console.error("Não foi possível abrir a nova aba. O navegador pode ter bloqueado.");
                }
            } else {
...
document.addEventListener("DOMContentLoaded", async function () {
    ...
    updateCarousel(await createRecommendationsItems(film.recommendations.items), "#Recommendations")
