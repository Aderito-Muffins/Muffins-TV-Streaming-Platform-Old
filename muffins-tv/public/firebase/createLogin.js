...
function displayError(message) {
  ...
  successContainer.style.display = "none"; // Oculta o contêiner de sucesso
  errorText.textContent = ""; // Limpa o texto de erro anterior

  // Define o texto da nova mensagem de erro
...
async function createLogin(email, password, fullname, phone) {
  try {
    ...
    if (data.code === 0) {
      esconderLoader();
      displaySuccess(data.message || "Cadastro realizado com sucesso!");

      // Redireciona para a página de verificação após 4,2 segundos
      setTimeout(() => {
...
async function verifyAccount(email, code) {
  try {
    const response = await fetch(
      `${config.API_URL}/users/verify-email?code=${code}&email=${email}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      }
