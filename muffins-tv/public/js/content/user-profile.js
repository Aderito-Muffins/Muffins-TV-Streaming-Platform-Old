...
function displayUserDetails(user) {
  ...
  const profileNameElement = document.querySelector(
    ".gen-user-profile .list-group-item:nth-child(1)"
  ); // Nome
  const profileEmailElement = document.querySelector(
    ".gen-user-profile .list-group-item:nth-child(2)"
  ); // Email
  const profilePhoneElement = document.querySelector(
    ".gen-user-profile .list-group-item:nth-child(3)"
  ); // Número de Telemóvel
  const profileRegistrationDateElement = document.querySelector(
    ".gen-user-profile .list-group-item:nth-child(4)"
  ); // Data de Cadastro
  const profileLastLoginElement = document.querySelector(
    ".gen-user-profile .list-group-item:nth-child(5)"
  ); // Último Login

  const formattedReg = new Date(user.registrationDate).toLocaleDateString(
    "pt-PT"
  );
  const formattedLastLog = new Date(user.lastLogin).toLocaleDateString("pt-PT");
  if (profileNameElement)
    profileNameElement.innerHTML = `<strong>Nome:</strong> ${
      user.fullName || "Nome não disponível"
    }`;
  if (profileEmailElement)
    profileEmailElement.innerHTML = `<strong>Email:</strong> ${
      user.email || "Email não disponível"
    }`;
  if (profilePhoneElement)
    profilePhoneElement.innerHTML = `<strong>Numero de Telemovel:</strong> ${
      user.phone || "Número de Telemóvel não disponível"
    }`;
  if (profileRegistrationDateElement)
    profileRegistrationDateElement.innerHTML = `<strong>Data de Cadastro:</strong> ${
      formattedReg || "Data de Cadastro não disponível"
    }`;
  if (profileLastLoginElement)
    profileLastLoginElement.innerHTML = `<strong>Último Login:</strong> ${
      formattedLastLog || "Último Login não disponível"
    }`;

  // Atualizar preferências do usuário, se necessário
  const preferencesGenresElement = document.querySelector(
    ".gen-user-preferences .list-group-item:nth-child(1)"
  ); // Gêneros Favoritos
  const preferencesNotificationsElement = document.querySelector(
    ".gen-user-preferences .list-group-item:nth-child(2)"
  ); // Notificações
  const preferencesLanguageElement = document.querySelector(
    ".gen-user-preferences .list-group-item:nth-child(3)"
  ); // Idioma Preferido
  const preferencesSubscriptionElement = document.querySelector(
    ".gen-user-preferences .list-group-item:nth-child(4)"
  ); // Plano de Assinatura
  const preferencesSpecialPackageElement = document.querySelector(
    ".gen-user-preferences .list-group-item:nth-child(5)"
  ); // Pacote Especial
  ...
  if (preferencesNotificationsElement) {
    preferencesNotificationsElement.innerHTML = `<strong>Notificações:</strong> ${
      user.preferences.notificationsEnabled ? "Ativadas" : "Desativadas"
    }`;
  }

  if (preferencesLanguageElement) {
    preferencesLanguageElement.innerHTML = `<strong>Idioma Preferido:</strong> ${
      user.preferences.preferredLanguage || "Não definido"
    }`;
  }
  ...
  if (preferencesSubscriptionElement) {
    ...
    if (planName && expirationDate) {
      const formattedExpirationDate = new Date(
        expirationDate
      ).toLocaleDateString("pt-PT"); // Formata a data para 'dd/mm/aaaa'
  ...
  if (preferencesSpecialPackageElement) {
    const formatted = new Date(
      user.specialPackage.expirationDate
    ).toLocaleDateString("pt-PT");
    preferencesSpecialPackageElement.innerHTML = `<strong>Pacote Especial:</strong> ${
      user.specialPackage?.isActive ? `Ativo até ${formatted}` : "Não Ativo"
    }`;
  }
}
